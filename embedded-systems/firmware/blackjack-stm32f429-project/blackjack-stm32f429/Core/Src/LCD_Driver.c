...
void LCD_Draw_Rectangle(uint16_t x, uint16_t y, uint16_t lenx, uint16_t leny, uint32_t color) {
	...
	for (uint16_t i = 0; i < lenx; i++) {
		...
		LCD_Draw_Pixel(x + i, y + leny - 1, color);
	...
	for (uint16_t i = 0; i < leny; i++) {
		...
		LCD_Draw_Pixel(x + lenx - 1, y + i, color);

	}

}

void LCD_Rectangle_Fill(uint16_t positionX, uint16_t positionY, uint16_t lenx, uint16_t leny, uint32_t color) {

	for (uint16_t y = 0; y < leny; y++) {

		for (uint16_t x = 0; x < lenx; x++) {

			LCD_Draw_Pixel(positionX + x, positionY + y,  color);
...
void LCD_Draw_String(uint16_t Xpos, uint16_t Ypos, char *string) {
